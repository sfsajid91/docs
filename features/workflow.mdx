...
```shell cURL
curl --request GET \
    --url '<EnvValue name="dashboardAPI"/>/scraper?api_key=<exampleToken>&workflow_id=cm1zwux5s000012i655qokya5'
```
...
```shell cURL
curl --request GET \
    --url '<EnvValue name="dashboardAPI"/>/workflow?api_key=<exampleToken>'
```
...
```shell cURL
curl --request GET \
    --url '<EnvValue name="dashboardAPI"/>/workflow/requests/${requestId}/?api_key=<exampleToken>'
```
...
```
<EnvValue name="dashboardAPI"/>/workflow/create/?api_key=<exampleApiKey>
```
...
```shell cURL
curl -X DELETE "<EnvValue name="dashboardAPI"/>/workflow/archive/${workflowId}?api_key=<exampleApiKey>
```
...
```shell cURL
curl -X POST "<EnvValue name="dashboardAPI"/>/workflow/unarchive/${workflowId}?api_key=<exampleApiKey>"
```
