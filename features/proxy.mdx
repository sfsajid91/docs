...
```shell cURL
curl --request GET \
    --url '<EnvValue name="dashboardAPI"/>/scraper?api_key=<exampleToken>&render=false&url=https%3A%2F%2Fexample.com%2F&proxy_type=residential'
```
...
```shell cURL
curl --request GET \
    --url '<EnvValue name="dashboardAPI"/>/scraper?api_key=<exampleToken>&render=false&url=https%3A%2F%2Fexample.com%2F&proxy_country=jp'
```
